  ...
  */
  {
    ...
    uri: `/services/data/v${apiVersionStr}/tooling/query/?q=SELECT+Id,LogUser.Name,LogLength,LastModifiedDate,Request,Operation,Status, DurationMilliseconds,SystemModstamp,StartTime,Location+FROM+apexlog+order+by+LastModifiedDate+desc`,
    method: 'GET',
    contentType: 'application/json',
    responeType: 'json',
  },
...
const getActionByName = (name, recordId = null) => {
  const action = actionList.find((action) => action.name === name);

  if (!action) return null;

  // Clone the action to avoid modifying the original object
  const actionWithReplacedUri = { ...action };

  // Replace <RecordId> placeholder if it exists in the uri and recordId is provided
  if (recordId && actionWithReplacedUri.uri.includes("<RecordId>")) {
    actionWithReplacedUri.uri = actionWithReplacedUri.uri.replace("<RecordId>", recordId);
  }

  return actionWithReplacedUri.uri;
};
